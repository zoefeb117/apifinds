...
const OutputPanel: React.FC<OutputPanelProps> = ({ schema, isProcessing }) => {
  ...
  return (
    <div className={`flex flex-col h-full transition-all duration-300 ${isFullscreen ? 'fixed inset-0 z-50 bg-white dark:bg-black' : ''}`}>
      <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center">
        <h2 className="text-lg font-semibold flex items-center gap-2">
          <CodeBracket className="h-5 w-5 text-black dark:text-white" />
          API Schema Output
        </h2>
        
        <div className="flex space-x-2">
          <button 
            className={`p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-900 transition-colors ${!schema ? 'text-gray-400 dark:text-gray-600 cursor-not-allowed' : ''}`}
            onClick={handleCopy}
            disabled={!schema}
            title="Copy to clipboard"
          >
            {copied ? <Check className="h-4 w-4 text-black dark:text-white" /> : <Copy className="h-4 w-4" />}
          </button>
        ...
        {isProcessing && !schema && (
          <div className="h-full flex flex-col items-center justify-center">
            <div className="relative w-16 h-16">
              ...
              <div className="absolute top-0 left-0 w-full h-full border-4 border-transparent border-t-black dark:border-t-white rounded-full animate-spin"></div>
            </div>
